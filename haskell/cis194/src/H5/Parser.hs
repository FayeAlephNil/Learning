...
module H5.Parser (parseExp) where
...
-- Parse a single white spaaaaaace character.
spaaaaaace :: Parser ()
spaaaaaace = State $ parseSpace
    where parseSpace [] = Nothing
          parseSpace s@(c:cs)
              | isSpace c = Just ((), cs)
              | otherwise = Nothing

-- Consume zero or more white spaaaaaace characters.
eatSpace :: Parser ()
eatSpace = const () <$> many spaaaaaace
